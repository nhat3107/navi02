import { useTheme } from '../hooks/useTheme';

interface ThemeToggleButtonProps {
  className?: string;
  layout?: 'default' | 'sidebar';
}

export function ThemeToggleButton({ className = '', layout = 'default' }: ThemeToggleButtonProps) {
  const { isDark, toggle } = useTheme();

  const icon = isDark ? (
    <svg
      width="18"
      height="18"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      className="text-amber-400"
      aria-hidden
    >
      <circle cx="12" cy="12" r="5" />
      <line x1="12" y1="1" x2="12" y2="3" />
      <line x1="12" y1="21" x2="12" y2="23" />
      <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
      <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
      <line x1="1" y1="12" x2="3" y2="12" />
      <line x1="21" y1="12" x2="23" y2="12" />
      <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
      <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
    </svg>
  ) : (
    <svg
      width="18"
      height="18"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden
    >
      <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
    </svg>
  );

  const ariaLabel = isDark ? 'Switch to light mode' : 'Switch to dark mode';

  if (layout === 'sidebar') {
    return (
      <button
        type="button"
        onClick={toggle}
        className={`app-sidebar__utility ${className}`.trim()}
        aria-label={ariaLabel}
        title={isDark ? 'Light mode' : 'Dark mode'}
      >
        <span className="app-sidebar__icon">{icon}</span>
        <span className="app-sidebar__label">{isDark ? 'Light mode' : 'Dark mode'}</span>
      </button>
    );
  }

  return (
    <button
      type="button"
      onClick={toggle}
      className={`app-top-nav__utility ${className}`.trim()}
      aria-label={ariaLabel}
    >
      {icon}
    </button>
  );
}

/** Theme toggle for auth / marketing screens — sits in the page corner, not over content. */
export function ThemeToggleCorner() {
  return (
    <ThemeToggleButton className="absolute top-4 right-4 z-10 sm:top-6 sm:right-6" />
  );
}
